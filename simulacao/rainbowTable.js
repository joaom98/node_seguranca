...
function criaHash(dado, algoritmo){
    return createHash(algoritmo).update(dado).digest('hex')
}
...
hashesVazadas.map( hashVazada => {
    rainbowTable.map( parGerado => {
        if (hashVazada === parGerado[1]){
            console.log(`Encontrado: a hash ${hashVazada} equivale à senha ${parGerado[0]}`)
        }
    } )
} )